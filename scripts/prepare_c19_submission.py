...
DEFAULT_TRAINED_MODEL = ROOT / "artifacts" / "models" / "full_data_c19_strong_date_shift_dropout_epoch58" / "model.pt"
DEFAULT_SUBMISSION_REPO = ROOT.parent / "track1_kybelix"
DEFAULT_SUBMISSION_CONFIG = ROOT / "configs" / "submission_c19_strong_date_shift_dropout.json"

CODE_FILES = [
    "configs/submission_c19_strong_date_shift_dropout.json",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare the GitLab submission repo with the trained C19 full-data checkpoint.")
...
def require_file(path: Path, label: str) -> None:
    if not path.exists():
        raise SystemExit(f"[prepare_c19_submission] missing {label}: {path}")
    if not path.is_file():
        raise SystemExit(f"[prepare_c19_submission] {label} is not a file: {path}")


def require_repo(path: Path) -> None:
    if not (path / ".git").exists():
        raise SystemExit(f"[prepare_c19_submission] target does not look like a git repo: {path}")
...
def main() -> None:
    ...
    require_file(trained_model, "trained C19 checkpoint")
    require_file(submission_config, "C19 submission config")
    ...
    for relative in CODE_FILES:
        if relative == "configs/submission_c19_strong_date_shift_dropout.json":
            continue
        copy_file(ROOT / relative, submission_repo / relative, dry_run=args.dry_run)

    if args.dry_run:
        print("[prepare_c19_submission] dry run complete; no files changed.")
        return

    if not args.skip_validate:
        validate_submission(submission_repo)
    print("[prepare_c19_submission] C19 files are ready in the GitLab submission repo.")
